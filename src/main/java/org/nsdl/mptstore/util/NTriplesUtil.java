...
import org.nsdl.mptstore.rdf.BNode;
...
public abstract class NTriplesUtil {
    ...
    /**
     * Consume the next URI or BNode reference.
     *
     * This will advance the reader through the next '&gt;' character.
     *
     * @param reader the reader to get characters from.
     ...
     *         is encountered.
     */
    public static String consumeURIorBNodeReference(final Reader reader, final int pos)
            throws IOException, ParseException {
     ...
     */
    public static Triple parseTriple(final String ntTriple, final String baseURI)
            throws ParseException {
        ...
        try {
            ...
            // subject
            String sString = consumeURIorBNodeReference(reader, i);
            SubjectNode subject = parseSubject(sString, baseURI);
            i += sString.length();
            ...
            ObjectNode object;
            try {
                object = parseObject(oString, baseURI);
            } catch (ParseException e) {
                throw new ParseException(e.getMessage(),
                        e.getErrorOffset() + i);
            }
     ...
     */
    public static SubjectNode parseSubject(final String ntSubject, final String baseURI)
            throws ParseException {
    	
        char first = ntSubject.charAt(0);

        if (first == '<') {
            return parseURIReference(ntSubject);
        } else if (first == '_') {
        	return parseBNode(ntSubject, baseURI);
        } else {
     ...
     */
    public static ObjectNode parseObject(final String ntObject, final String baseURI)
            throws ParseException {

        return (ObjectNode) parseNode(ntObject, baseURI);
    }
     ...
     * @throws ParseException if the input syntax is incorrect
     */
    public static Node parseNode(final String ntNode, final String baseURI)
            throws ParseException {
        ...
        if (first == '"') {
        	...
        	return parseBNode(ntNode, baseURI);
        } else {
            throw new ParseException(EXPECTED_QL, 0);
        }
    }
     ...
     * @throws ParseException if the input syntax is incorrect
     */
    public static BNode parseBNode(final String s, String baseURI)
            throws ParseException {
        ...
        try {
            return new BNode(s.substring(1, s.length() - 1), baseURI);
        } catch (URISyntaxException e) {
            throw new ParseException(EXPECTED_ABS_URI, 1);
        }
    }
